...
from dataset.Custom_Dataset import DogCat
...
trainset=DogCat('../data/train',transform=transform_train, train=True, val=False, test=False)
valset  =DogCat('../data/train',transform=transform_val, train=False, val=True, test=False)
...
def train(epoch):
	...
	for batch_idx,(img,label) in enumerate(trainloader):
		image,label = img.cuda(), label.cuda()
		optimizer.zero_grad()
		out=model(image)
